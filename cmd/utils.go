...
func checkAPICompatibility(c *deis.Client, err error, wOut io.Writer) error {
	if err == deis.ErrAPIMismatch {
		fmt.Fprintf(wOut, `!    WARNING: Client and server API versions do not match. Please consider upgrading.
!    Client version: %s
!    Server version: %s
